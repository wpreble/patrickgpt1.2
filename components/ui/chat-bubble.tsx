...
export function ChatBubbleMessage({ variant = "received", isLoading, className, children }: ChatBubbleMessageProps) {
  return (
    <div
      className={cn(
        "rounded-lg p-3 text-sm", // Common styles
        variant === "sent"
          ? "bg-primary text-primary-foreground" // Sent messages
          : "bg-secondary text-secondary-foreground", // Received messages (using secondary for better contrast with background)
        className,
      )}
    >
      {isLoading ? (
...
export function ChatBubbleAvatar({ src, fallback = "AI", className }: ChatBubbleAvatarProps) {
  return (
    <Avatar className={cn("h-8 w-8", className)}>
      <AvatarImage src={src || "/placeholder.svg"} alt={fallback} /> {/* Ensure alt is passed */}
